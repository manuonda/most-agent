...
- `bin/mantis-api.sh` — single entry point for every Mantis REST call (`issue`, `notes`, `status`, `whoami`, `download`). It resolves `MANTIS_API_TOKEN` (env first, then settings files) so the skills never inline `curl`/`python3`. `install.sh` links it into `~/.claude-most/bin/` and adds the allow rule `Bash(~/.claude-most/bin/mantis-api.sh:*)`, which is what keeps the skills prompt-free — the skills must invoke it with exactly that `~/.claude-most/bin/mantis-api.sh` prefix or the permission match breaks. `download` fetches an issue attachment to disk (used by `mantis_develop` to read documentation attached to the issue); it refuses to send the token to any host other than `$MANTIS_BASE_URL`.
- `bin/git-credential-env.sh` — git credential helper for `git.grupomost.com`, backed by `GIT_GRUPOMOST_USER` / `GIT_GRUPOMOST_TOKEN` env vars instead of an on-disk store or an interactive prompt. `mantis_develop` needs `git fetch origin test` to authenticate non-interactively before creating the worktree; the GEINS remotes are HTTPS with no credential storage on a fresh machine, so without this the fetch would hang/fail. `install.sh` wires it up via `git config --global credential."https://git.grupomost.com".helper`, scoped to that host only — it never touches credentials for any other remote, and implements only the `get` operation (`store`/`erase` are no-ops, there is nothing on disk to update).
...
- `MANTIS_API_TOKEN` is per-developer, stored in each consuming project's `.claude/settings.local.json` under `env` — never committed, never echoed in chat. Skills must keep this contract. Same for `JENKINS_USER` / `JENKINS_API_TOKEN` (generated at `<jenkins>/user/<id>/security/`): per-developer on purpose, so the Jenkins build log records who triggered what. Same for `GIT_GRUPOMOST_USER` / `GIT_GRUPOMOST_TOKEN` (a personal access token from `git.grupomost.com`, not the account password), consumed by `bin/git-credential-env.sh`.
